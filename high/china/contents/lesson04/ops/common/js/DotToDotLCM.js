  ...
  if (isWithinX && isWithinY) {
    if (item.dataset.type == "answer" && this.activeBool) {
      ...
      if(this.selQues.val == this.selAns.val) {
        console.log("정답 감지");
        // 애니메이션 함수로 작동
        ansLiner[item.dataset.val - 1] = (new AnsLine(this.selQues.x, this.selQues.y, this.selAns.x, this.selAns.y));
        ...
        $(".reset_btn").eq(item.dataset.val - 1).show();
        $(".answer_btn").eq(item.dataset.val - 1).hide();
      }
    }
  }
});
...
testObj.forEach((item, key) => {
  if(item.dataset.type == "question") {
    ...
    const itemCenterX = (itemRect.x + itemRect.width / 2);
    const itemCenterY = (itemRect.y + itemRect.height / 2);
    quesInfo[item.dataset.val - 1] = {val: item.dataset.val, x: itemCenterX, y: itemCenterY};
  } else {
    const itemRect = item.getBoundingClientRect();
    const itemCenterX = (itemRect.x + itemRect.width / 2);
    const itemCenterY = (itemRect.y + itemRect.height / 2);
    ansInfo[item.dataset.val - 1] = {val: item.dataset.val, x: itemCenterX, y: itemCenterY};
  };
