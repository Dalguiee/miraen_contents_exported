...
const dotToDotActivate = () => {
	...
	const handleStartDotClick = (x, y) => {
		startDots.each((_, dot) => {
			...
			const dotX = dotRect.left + dotRect.width / 2 - canvas.getBoundingClientRect().left;
			const dotY = dotRect.top + dotRect.height / 2 - canvas.getBoundingClientRect().top;
	...
	const handleMove = e => {
		if (isDragging) {
			...
			const { x, y } = getClientCoords(e);
			if (startDot) {
				drawLine(startX, startY, x, y, '#498f63', 5);
			} else if (endDot) {
				drawLine(x, y, startX, startY, '#498f63', 5);
			}
		}
	};
		...
		if (isDragging) {
			...
			if (startDot) {
				endDots.each((_, dot) => {
					const $dot = $(dot);
					const dotRect = dot.getBoundingClientRect();
					const dotCenterX = dotRect.left + dotRect.width / 2 - canvas.getBoundingClientRect().left;
					const dotCenterY = dotRect.top + dotRect.height / 2 - canvas.getBoundingClientRect().top;
					...
					if (Math.abs(x - dotCenterX) < 20 && Math.abs(y - dotCenterY) < 20) {
						if (startDot.dataset.start === dot.dataset.end) {
							...
							$('.answer_btn')
								.eq(dot.dataset.end - 1)
								.hide();
							$('.reset_btn')
								.eq(dot.dataset.end - 1)
								.show();
							$(`[data-answer="${dot.dataset.end}"]`).show();
							try {
								// audioPlay(audio, [`./media/mp3/showAnswer.mp3`]);
							} catch (error) {}
							//=============================================================
							// 이미 연결된 선이 아닌 경우에만 추가
							const alreadyConnected = lines.some(line => (line.startDot === startDot && line.endDot === dot) || (line.startDot === dot && line.endDot === startDot));
				...
				startDots.each((_, dot) => {
					...
					if (Math.abs(x - dotCenterX) < 20 && Math.abs(y - dotCenterY) < 20) {
						if (endDot.dataset.end === dot.dataset.start) {
							...
							//=============================================================
							// 이미 연결된 선이 아닌 경우에만 추가
							const alreadyConnected = lines.some(line => (line.startDot === dot && line.endDot === endDot) || (line.startDot === endDot && line.endDot === dot));
	...
	const checkCompletion = () => {
		...
		// 각 start_dot에 대해 하나의 end_dot 연결 필요
		const totalExpectedConnections = totalStartDots;
	...
	const drawDots = () => {
		startDots.each((_, dot) => {
			...
			ctx.fillStyle = connectedDots.has(dot) ? 'rgba(0,0,0,0)' : '#498f63'; // 점 색깔
			ctx.fill();
		});
		...
		endDots.each((_, dot) => {
			...
			ctx.beginPath();
			ctx.arc(dotX, dotY, 5, 0, 2 * Math.PI);
			ctx.fillStyle = connectedDots.has(dot) ? 'rgba(0,0,0,0)' : '#498f63';
			ctx.fill();
		});
	};
	...
	const drawLines = () => {
		...
		lines.forEach(line => {
			// 연결된 선 색상과 굵기
			drawLine(line.startX, line.startY, line.endX, line.endY, '#E22F61', 5);
		});
	};
...
const canversReset = () => {
	...
	$('.line-content-wrap canvas').first().remove();
	// <canvas> 요소를 다시 생성하여 .line-content-wrap의 맨 위에 추가
	$('.line-content-wrap').prepend('<canvas class="canvas" width="343" height="348" id="canvas"></canvas>');
};
